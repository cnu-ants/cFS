...
set(LL_APP_LIST "")
macro(exll_cfe_app APP_NAME APP_SRC_FILES)
  list(APPEND LL_APP_LIST ${APP_NAME})
endmacro()
##################################################################
#
...
list(FIND LL_APP_LIST "${APP_NAME}" _idx)
if (NOT _idx EQUAL -1)  
  set(ABS_SRC_LIST "")
  foreach(SRC ${APP_SRC_FILES} ${ARGN})
    get_filename_component(SRC_ABS ${SRC} ABSOLUTE)
    #list(APPEND ABS_SRC_LIST ${SRC_ABS})
    #endforeach()
    get_filename_component(FILENAME "${SRC_ABS}" NAME)
  #foreach(SRC_ABS ${ABS_SRC_LIST})
    add_custom_command(
      TARGET ${APP_NAME}
      POST_BUILD
      COMMAND ${CMAKE_C_COMPILER} -S -emit-llvm ${SRC_ABS} -o
      ${CMAKE_CURRENT_BINARY_DIR}/${FILENAME}.ll 
...
#function(exll_cfe_app APP_NAME APP_SRC_FILES)
##
##  # currently this will build an app with either static linkage or shared/module linkage,
##  # but this does not currently support both for a single arch (could be revised if that is needed)
#  if (APP_DYNAMIC_TARGET_LIST)
...
##  )
##  set(INCLUDE_FLAGS "")	
##  foreach(dir ${CURRENT_INCLUDE_DIRS})
##    list(APPEND INCLUDE_FLAGS -I${dir})
##  endforeach()
##
##  add_custom_command(        
##  OUTPUT ${APP_NAME}.ll
##  COMMAND ${CMAKE_C_COMPILER} -S -emit-llvm ${ABS_SRC_LIST} -o
##  ${APP_NAME}.ll -MD -MT ${CMAKE_CURRENT_BINARY_DIR}/${APP_SRC_FILES}.ll -MF ${CMAKE_CURRENT_BINARY_DIR}/${APP_SRC_FILES}.d $(C_DEFINES) $(C_INCLUDES) $(C_FLAGS)
##  DEPENDS ${ABS_SRC_LIST})
##  
##  #add_custom_target(${APP_NAME}_ir ALL DEPENDS ${APP_NAME}.ll)
##
##
#endfunction(exll_cfe_app)
#
